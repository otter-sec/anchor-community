...
use rbac_utils::{constants::IDENTITY_REGISTRY_ID, CREATE_IDENTITY_ACCOUNT_IX};
...
pub fn handler(ctx: Context<CreateIdentityAccount>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::CREATE_IDENTITY_ACCOUNT,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&CREATE_IDENTITY_ACCOUNT_IX),
