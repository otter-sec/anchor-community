...
use rbac_utils::{
    constants::{IDENTITY_METADATA_REGISTRY_ID, IDENTITY_REGISTRY_ID},
    REGISTER_INVESTOR_IX,
};
...
pub fn handler(ctx: Context<RegisterInvestor>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::REGISTER_INVESTOR,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&REGISTER_INVESTOR_IX),
