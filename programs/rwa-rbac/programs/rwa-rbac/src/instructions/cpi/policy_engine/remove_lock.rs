...
use rbac_utils::{constants::POLICY_ENGINE_ID, REMOVE_LOCK_IX};

use crate::{controller_seeds, error::ErrorCode, state::*, verify_medici_signer};

#[derive(Accounts)]
pub struct RemoveLock<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<RemoveLock>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::REMOVE_TOKEN_LOCK,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&REMOVE_LOCK_IX),
