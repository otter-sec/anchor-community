...
use rbac_utils::{constants::POLICY_ENGINE_ID, CHANGE_ISSUANCE_POLICIES_IX};
...
pub fn handler(ctx: Context<ChangeIssuancePolicies>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::CHANGE_ISSUANCE_POLICIES,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&CHANGE_ISSUANCE_POLICIES_IX),
