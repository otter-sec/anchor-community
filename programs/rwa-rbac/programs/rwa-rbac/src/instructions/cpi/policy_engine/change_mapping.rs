...
use rbac_utils::{constants::POLICY_ENGINE_ID, CHANGE_MAPPING_IX};
...
pub fn handler(ctx: Context<ChangeMapping>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::CHANGE_MAPPING,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&CHANGE_MAPPING_IX),
