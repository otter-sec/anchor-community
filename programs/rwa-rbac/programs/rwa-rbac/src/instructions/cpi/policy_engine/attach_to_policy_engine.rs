...
use rbac_utils::{constants::POLICY_ENGINE_ID, ATTACH_TO_POLICY_ENGINE_IX};
...
pub fn handler(ctx: Context<AttachToPolicyEngine>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::ATTACH_TO_POLICY_ACCOUNT,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&ATTACH_TO_POLICY_ENGINE_IX),
