...
use rbac_utils::{constants::POLICY_ENGINE_ID, DETACH_FROM_POLICY_ENGINE_IX};
...
pub fn handler(ctx: Context<DetachFromPolicyEngine>, cpi_data: Vec<u8>) -> Result<()> {
    ...
    verify_medici_signer(
        ...
        MediciFlags::DETACH_FROM_POLICY_ACCOUNT,
    )?;

    // Verify instruction discriminator in cpi_data
    require!(
        cpi_data.starts_with(&DETACH_FROM_POLICY_ENGINE_IX),
